...
function Repositories() {
  return (
    <div className={`bg-white  w-full md:w-full h-[100vh] md:overflow-scroll`}>
      <div className="md:ml-4  p-2 border border-gray-300 mr-4 mt-8 rounded-lg flex flex-col gap-1">
        ...
        <div className="flex flex-col justify-center gap-1">
          {repoData.map((item) => (
            <div key={item.name} className="flex flex-col gap-3  bg-white md:p-5 p-3 transition-all ease-linear duration-50 border-b hover:bg-[#f5f5f5]">
              ...
              <div className="flex flex-row gap-5 md:gap-10">
                <div className="flex flex-row gap-2 items-center">
                  ...
                  <div className="rounded-[5rem] bg-[rgb(20,112,239)] w-2 h-2"></div>
                </div>
                <div className="flex gap-2 flex-row items-center">
                  <img src="/database.png" alt="" />
                  <p>{item.size}</p>
                </div>
                <p>Updated {item.updates} days ago</p>
              </div>
            </div>
          ))}
        </div>
